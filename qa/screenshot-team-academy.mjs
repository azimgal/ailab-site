...
const MAIN    = 'file:///C:/Users/saler/OneDrive/%D0%A0%D0%B0%D0%B1%D0%BE%D1%87%D0%B8%D0%B9%20%D1%81%D1%82%D0%BE%D0%BB/work/ai%20laboratory/prototype/index.html';
const ACADEMY = 'file:///C:/Users/saler/OneDrive/%D0%A0%D0%B0%D0%B1%D0%BE%D1%87%D0%B8%D0%B9%20%D1%81%D1%82%D0%BE%D0%BB/work/ai%20laboratory/prototype/academy/index.html';
...
async function shootSection(url, sectionSelector, outFile, label) {
  killChrome();
  await sleep(700);
  ...
  await send(ws, 'Runtime.evaluate', { expression: `document.querySelector('${sectionSelector}')?.scrollIntoView({block:'start'})` });
  await sleep(400);

  const { result } = await send(ws, 'Runtime.evaluate', {
    expression: `(() => { const el = document.querySelector('${sectionSelector}'); const r = el.getBoundingClientRect(); return JSON.stringify({top:r.top,h:el.scrollHeight,scrollY:window.scrollY}); })()`
  });
...
await shootSection(MAIN,    '#ailab-team',    'main-team-desktop.png',    'Team section');
await shootSection(ACADEMY, '.ac-hero',       'academy-hero-desktop.png', 'Academy hero');
